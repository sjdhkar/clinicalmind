...
@Component({
      ...
      <div class="messages" #messageContainer>
        ...
        @for (msg of messages(); track msg.id) {
          <div class="message" [class.user]="msg.role === 'user'" [class.assistant]="msg.role === 'assistant'">
            ...
            <div class="bubble">
              <p [appStreamText]="msg.content" [isStreaming]="msg.streaming" class="message-text"></p>
  ...
  `,
  styles: [`
    .chat-panel { display:flex;flex-direction:column;height:100%;background:var(--color-background-primary); }
    ...
    .avatar { width:32px;height:32px;border-radius:50%;display:flex;align-items:center;justify-content:center;font-size:11px;font-weight:500;background:var(--color-background-info);color:var(--color-text-info);flex-shrink:0; }
    .user-avatar { background:var(--color-background-secondary);color:var(--color-text-secondary); }
    .bubble { max-width:70%;background:var(--color-background-secondary);border-radius:12px;padding:.75rem 1rem; }
    ...
    .message-meta { display:flex;gap:8px;margin-top:8px;font-size:11px;color:var(--color-text-tertiary); }
    .model-badge { background:var(--color-background-tertiary);padding:1px 6px;border-radius:4px; }
    ...
    .suggestion-chip { text-align:left;padding:.5rem .75rem;background:var(--color-background-secondary);border:0.5px solid var(--color-border-tertiary);border-radius:8px;cursor:pointer;font-size:13px;color:var(--color-text-secondary); }
    .thinking { display:flex;gap:4px;padding:.5rem 1rem; }
    .dot { width:6px;height:6px;border-radius:50%;background:var(--color-text-tertiary);animation:bounce 1.2s infinite; }
    .dot:nth-child(2) { animation-delay:.2s; }
    .dot:nth-child(3) { animation-delay:.4s; }
    .input-area { display:flex;gap:8px;padding:1rem;border-top:0.5px solid var(--color-border-tertiary); }
    .query-input { flex:1;resize:none;border:0.5px solid var(--color-border-secondary);border-radius:8px;padding:.5rem .75rem;font-size:14px;background:var(--color-background-primary);color:var(--color-text-primary); }
    .query-input:focus { outline:none;border-color:var(--color-border-primary); }
    .send-btn { padding:0 1rem;background:var(--color-background-info);color:var(--color-text-info);border:none;border-radius:8px;cursor:pointer;font-size:14px; }
...
export class ChatComponent {
  ...
  // FIX: typed as Event (Angular keydown binding), cast internally
